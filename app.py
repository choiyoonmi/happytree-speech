...
#   구글 직통은 수업시작 때 1.5~2초+동시몰림 폭주(최대 11초+) → 로딩 길고 재로그인 튕김(2026-09-18 원장 제보).
#   워커는 로그인을 D1 로 처리(~0.8초, 폭주 없음)하고, 로그인 아닌 액션(rosterInfo 등)은 캐시/구글로 넘긴다.
ACCOUNT_PROXY = "https://happytree-math-proxy.white21040.workers.dev/"
STUDENT_ACCOUNT_API = os.environ.get("STUDENT_ACCOUNT_API", ACCOUNT_PROXY)
# ★환경변수에 옛 구글 exec 주소가 남아 있어도 빠른 워커로 돌린다(워커가 구글로 넘겨주므로 안전).
#   이렇게 안 하면 Render 환경변수 설정이 코드 기본값을 덮어써서 계속 느린 구글로 간다.
if "script.google.com" in STUDENT_ACCOUNT_API:
    STUDENT_ACCOUNT_API = ACCOUNT_PROXY
...
def treetalk_points(ym: str = "", days: str = ""):
    ...
    def _due_md(due):
        try:
            ...
            pass
        return None
    # ★마감일이 속한 '주' 기준으로만 센다(원장 2026-09-22): 마감일을 지나도 그 주간 안에 완료하면
    #   점수를 주고, 지난 주 마감 과제를 이번 주에 늦게 완료하면 이번 주엔 안 준다(그 과제의 마감주로 감).
    #   → dueDate 의 M/d 가 이번 주(days) 안이면 인정. 제출/완료 시각은 따로 안 본다(마감 지나도 그 주면 OK).
    out = {}
    ...
    for sid in ids:
        ...
        for aid, sub in subs.items():
            ...
            if dmd is not None:
                if not in_period(dmd):
                    continue
                # ★마감일이 그 주간이면, 마감일을 지나서 완료했어도 그 주 점수로 인정(원장 2026-09-22)
        ...
        for aid, rec in vocab.items():
            ...
            if dmd is not None:
                if not in_period(dmd):
                    continue
                # ★마감일이 그 주간이면, 마감일을 지나서 했어도 그 주 점수로 인정(원장 2026-09-22)
...
def _pa_words(pa):
    ...
    recognizedText 는 참조문장에 맞춰 정렬돼 있어 못 믿는다(아래 assess_with_sdk 주석 참고)."""
    out = []
    try:
...
def assess_with_sdk(wav_path: str, reference: str, debug: bool = False):
    ...
    # ── 읽기가 느린 학생이 "녹음이 자꾸 끊긴다"고 하는 자리 ──────────────────
    # Azure 는 말이 잠깐 멎으면 "발화가 끝났다"고 보고 인식을 닫는다. 그 기본값이
    # **0.5초**다. 또박또박 읽거나 낱말 사이에서 한 번 쉬는 아이는 거의 매번 거기서
    # 잘리고, **잘린 뒤쪽은 통째로 Omission** 이 되어 completeness·fluency 가 무너진다.
    # 학생 눈에는 "끝까지 읽었는데 점수가 엉망"으로 보인다.
    #   오디오는 **파일**로 보내므로 이 값을 늘려도 손해가 없다 — 파일이 끝나면 어차피 끝난다.
    #   빨리 읽는 아이에게는 아무 영향이 없다.
    # SEG  : 문장 가운데 쉬어도 되는 시간 (Azure 허용 100~5000ms)
    # INIT : 녹음 시작 뒤 첫 소리까지 기다리는 시간 (기본 5초 — 느린 아이는 여기서도 잘린다)
    # MAXT : 한 덩어리로 인식하는 최대 길이 (기본 20초)
    _SEG  = os.environ.get("SPEECH_SEGMENTATION_MS", "3000")
    _INIT = os.environ.get("SPEECH_INITIAL_SILENCE_MS", "15000")
    _MAXT = os.environ.get("SPEECH_SEGMENT_MAX_MS", "60000")
    for _pid, _v in (
        (speechsdk.PropertyId.Speech_SegmentationSilenceTimeoutMs, _SEG),
        (speechsdk.PropertyId.SpeechServiceConnection_InitialSilenceTimeoutMs, _INIT),
        (speechsdk.PropertyId.Speech_SegmentationMaximumTimeMs, _MAXT),
    ):
        # SDK 판이 바뀌어 어느 하나를 안 받아도 채점 자체는 굴러가야 한다
        try:
            speech_config.set_property(_pid, str(_v))
        except Exception as _e:
            print(f"[assess] 무음 설정 건너뜀 {_pid}: {_e}")
